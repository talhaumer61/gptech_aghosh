                                        ...
                                        while($rowClass = mysqli_fetch_array($sqllms)){
                                            ...
                                            if($mobilenum1 !='' &&  strlen($mobilenum1) == 12 ) {
                                                ...
                                                $message_text = "Dear " . $rowClass['std_name'] . "\n" .
                                                                "Kindly ensure to submit your school fee payment month of " . get_monthtypes($rowClass['id_month']) . "-" . date('Y', strtotime(cleanvars($rowClass['due_date']))) . " before due date to avoid any inconvenience. If the school fee is not paid by due date " . date('d-m-Y', strtotime($rowClass['due_date'])) . " fine Rs 300 will be imposed with monthly fee.\n\n" .
                                                                "Scan to Pay from Any Mobile Banking App\n" .
                                                                "Challan Number: " . $rowClass['challan_no'] . "\n\n\n" .
                                                                "Challan Amount: Rs. " . number_format($rowClass['total_amount']) . "\n\n\n" .
                                                ...
                                                curl_setopt_array($curl, array(
                                                    ...
                                                    CURLOPT_POSTFIELDS => json_encode(array(
                                                        'appkey'  => 'b3af15ce-ade3-469a-9210-a6516da8b1f7',
                                                        'authkey' => '8YfWfi4tSVVhkLYuo3KYcd0EMHutMAReySWFfRGoGpBACaZ6DT',
                                                        'to'      => $mobilenum1,
                                                        'message' => $message_text,
                                                        'file'    => 'https://aghosh.gptech.pk/uploads/feechallans/' . $rowClass['challan_no'] . '.pdf'
                                                    )),
                                                ));
