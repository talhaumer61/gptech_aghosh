<?php 
	if(isset($dataArray['ReferenceNumber']) && isset($dataArray['BankAccountCode']) && isset($dataArray['X_API_KEY'])){
		...
		$queryChallan = $dblms->querylms("SELECT f.status, f.id_type, f.challan_no, f.due_date, f.total_amount, f.paid_amount, f.id_std, f.id_campus, f.id_month, f.inquiry_formno, f.issue_date,
																cls.class_name, cm.campus_customercode, std.*, COALESCE(std.std_name, q.name) AS student_name, q.name, q.form_no, don.donor_name, cs.section_name, s.session_name
																FROM ".FEES." f
		...
		if(mysqli_num_rows($queryChallan) == 1) {
						...
						foreach($monthtypes as $month):
							if($valueChallan['id_month']==$month['id']){
								if($valueChallan['due_date'] < date('Y-m-d')){ 
									$amount = (($valueChallan['total_amount'] + $latefee) - $valueChallan['paid_amount']);
								} else {
									$amount = ($valueChallan['total_amount'] - $valueChallan['paid_amount']);	
								}
			...
			$data['LateAmount'] 				= ($grandTotal+$bankCharges);
			...
			$data['StudentName'] 				= $valueChallan['student_name'];
