...
$sqllms  = $dblms->querylms("SELECT f.id, f.status, f.id_type, f.id_month, f.yearmonth, f.challan_no, f.id_session, f.id_class, f.id_section, f.inquiry_formno, f.id_std, f.narration,
											...
											c.class_id, c.class_name, c.id_classgroup, f.paid_date, 
											...
											AND f.status = '1' AND f.yearmonth = '".$yearmonth."' 
											AND f.is_deleted != '1'  
											ORDER By f.id ASC");
                                    ...
                                    <tbody>';
                                        while($rowClass = mysqli_fetch_array($sqllms)){
                                            ...
                                            if($mobilenum1 !='' &&  strlen($mobilenum1) == 12 ) {
                                                ...
                                                $message_text = "Dear " . $rowClass['std_name'] . "\n" .
                                                                "Your Fee Challan No " . $rowClass['challan_no'] . " of " . get_monthtypes($rowClass['id_month']) . "-" . date('Y', strtotime(cleanvars($rowClass['due_date']))) . " has been paid Dated " . date('d-m-Y', strtotime(cleanvars($rowClass['paid_date']))) . ".\n\n" .
                                                                "https://aghosh.gptech.pk/feechallanprintwa.php?id=" . $rowClass['challan_no'] . "\n\n\n" .
                                                ...
                                                curl_setopt_array($curl, array(
                                                    ...
                                                    CURLOPT_POSTFIELDS => array('appkey' => 'b3af15ce-ade3-469a-9210-a6516da8b1f7','authkey' => '8YfWfi4tSVVhkLYuo3KYcd0EMHutMAReySWFfRGoGpBACaZ6DT','to' => $mobilenum1,
