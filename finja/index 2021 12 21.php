		...
		if($data_arr['method']=="getChallanInfo"){ 
			...
			if($data_arr['username'] == ''){
				...
				if(mysqli_num_rows($sqllmsuser) == 1){
					...
					//-----------------------------------------------------
					$sqllmschallan	= $dblms->querylms("SELECT f.status, f.id_type, f.challan_no, f.due_date, f.total_amount, f.id_campus,  
					...
					if (mysqli_num_rows($sqllmschallan) == 1) {
						...
						} elseif($rowchallan['status'] == '2' && ($lastdate>=date("Y-m-d"))) {
						...
						if($rowchallan['status'] == '2' && $lastdate < date("Y-m-d")) {
							...
							if($totalrecived >= $dueamount) {
								...
								if($rowuser['api_allowpartial'] == 1) { 
									...
									if($rowtransamount['totaltransamount'] == $dueamount || $rowtransamount['totaltransamount'] >= $dueamount) {
	
										$sqllmsupdate  = $dblms->querylms("UPDATE ".FEES." SET 
																			status    	= '1'
																		, paid_date		= '".date('Y-m-d', strtotime(cleanvars($data_arr['transDate'])))."'
																		, paid_amount	= '".cleanvars($transamount- BANKCHARGES)."'
																		, pay_mode		= '4' 
																		, date_modify	= NOW()
