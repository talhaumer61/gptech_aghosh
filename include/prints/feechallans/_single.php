...
<table width="99%" border="0" class="page " id="myTable" cellpadding="7" cellspacing="10" align="center" style="border-collapse:collapse; margin-top:0px;">
	...
	QRcode::png($link, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
		...
		for($ifee = 1; $ifee<=3; $ifee++){
			...
			// --- PRE-PROCESS STAMP HTML PATTERN BEFORE OUTPUT COPIES ---
			$stampHtml = '';
			if ($feercord['status'] == '1' || $feercord['status'] == '4') {
				$stampHtml = '
				<div class="paid-stamp-box">
					<span class="stamp-text-meta">DATE: '.$feercord['paid_date'].'</span><br>
					'.($feercord['status'] == '1' ? '<span class="stamp-title-large">PAID</span>' : '<span class="stamp-title-medium">PARTIAL PAID</span>').'<br>
					<span class="stamp-text-meta">TOTAL: Rs '.number_format($feercord['paid_amount']).'.00</span><br>
					<span class="stamp-text-meta">COLLECTOR: '.get_paymethod($feercord['pay_mode']).'</span>
				</div>';
			}

			if($feercord['id_type']=='2'){
				$sqlusername  = $dblms->querylms("SELECT adm_username FROM ".ADMINS." 
												WHERE adm_id = '".cleanvars($feercord['id_loginid'])."' LIMIT 1");
				$valusername = mysqli_fetch_array($sqlusername);
				echo '
				<td width="341" '.$rightborder.' class="challan-cell '.$clspaid.'">
					'.$stampHtml.'
					<table style="border-collapse:collapse; margin-top: 5px; margin-bottom: 5px;" width="100%" border="0">
						<tr>
							<td>
								<img src="uploads/ags-logo.png" alt="AGHOSH COMPLEX LAHORE" style="width:90px; height: 90px; text-align: left; vertical-align: middle;">
								<br>
							</td>
							<td>
				...
				<td width="341"  '.$rightborder.' class="challan-cell '.$clspaid.'">
					...
					<div style="font-size:13px; margin-top:5px;">
						...
						<tr>
							<td style="text-align: center;"><img src="uploads/1-link.jpg" style="width:35px; height: 35px; vertical-align: middle; "></td>
							<td style="font-size:14px; font-weight:600;">All Mobile Banking Payments:<br>1 Bill  Invoice ID: NA</td>
						</tr>
						<tr>
							<td style="text-align: center;"><img src="uploads/ubl.png" style="width:35px; height: 35px; vertical-align: middle; "></td>
							<td style="font-size:14px; font-weight:600; padding:10px;">UBL All Over Pakistan ('.$ublAccount.')<br>Challan No: '.$feercord['challan_no'].'</td>
