...
<script type="text/javascript">
    // 1. GLOBAL MODAL OVERRIDES (Executes immediately)
    ...
    // 2. RE-ENGINEERED DYNAMIC INITIALIZATION FUNCTION
    function initializeDynamicSelect2(container) {
        $(container).find(\'[data-plugin-selectTwo]\').each(function() {
            var $this = $(this);
            
            // If it was already initialized prematurely by the theme template, destroy it safely first
            if ($this.hasClass("select2-hidden-accessible") || $this.data(\'select2\')) {
                $this.select2(\'destroy\');
            }
            
            // Build the option configuration map
            var options = {
                width: $this.attr(\'data-width\') || \'100%\',
                theme: \'bootstrap\' // Ensures formatting fits your theme classes
            };
            
            if ($this.attr(\'data-minimum-results-for-search\') === \'Infinity\') {
                options.minimumResultsForSearch = Infinity;
            }
            
            // Fire up the clean Select2 element
            $this.select2(options);
        });
    }

    // 3. ROBUST GLOBAL AJAX WATCHER
    // Listens globally for any completed AJAX requests, giving the DOM 150ms to print out the <option> loops
    $(document).ajaxComplete(function(event, xhr, settings) {
        setTimeout(function() {
            initializeDynamicSelect2(document);
        }, 150); 
    });

    // 4. DOM READY FUNCTIONS
    jQuery(document).ready(function($) {
        // Run on base page elements on initial load
        initializeDynamicSelect2(document);

        // DataTables Initialization
        if ($(\'.table_default\').length) {
