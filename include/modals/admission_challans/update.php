								...
								<div class="col-md-12">
									...
									<select class="form-control" data-plugin-selectTwo data-width="100%" data-minimum-results-for-search="Infinity" name="id_month" name="id_month" required>
										...
										foreach($monthtypes as $month){
											echo'<option value="'.$month['id'].'" '.($month['id'] == $rowsvalues['id_month'] ? 'selected' : '').'>'.$month['name'].' - '.date('Y').'</option>';
										}
										echo '
									</select>
								</div>
									...
									<div class="col-md-4">
										<div class="form-group mt-sm">
											<div class="col-md-12">
												...
												<input type="number" id="amount" name="amount[]" class="form-control cats" required title="Must Be Required" value="'.$valuefeeprt['amount'].'" '. $readable .' />
											</div>
										</div>
									</div>
									';
