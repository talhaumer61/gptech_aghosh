...
if(($_SESSION['userlogininfo']['LOGINTYPE']  == 1) || Stdlib_Array::multiSearch($_SESSION['userroles'], array('right_name' => '71', 'view' => '1'))){ 
    ...
    $sqllmsStudent	= $dblms->querylms("SELECT s.std_id, s.std_name, s.std_status, s.id_class, s.std_fathername, s.id_session, s.admission_formno, 
											s.std_photo, s.std_whatsapp, s.std_fathercnic, s.std_regno, c.class_name, se.section_name, ss.session_name, fs.id AS idsetup
                                            FROM ".STUDENTS." s		   
...
// Get all Student Concessions
    ...
    $sqllmsAcc	= $dblms->querylms("SELECT f.id, f.status, f.id_month, f.challan_no, f.issue_date, f.paid_date, 
                                    ...
                                    AND (f.id_std = '".cleanvars($_GET['id_std'])."' OR f.inquiry_formno = '".cleanvars($valStdDet['admission_formno'])."')
                                    AND f.is_deleted != '1'
                                ...
                                <tbody>';
                                    ...
                                    while($valAccounts = mysqli_fetch_array($sqllmsAcc)) {
                                        ...
                                        <tr>
                                            ...
                                            <td>'.get_monthtypes($valAccounts['id_month']).'-'.date('Y' , strtotime(cleanvars($valAccounts['due_date']))).'</td>
