...
//-----------------------------------------------------
$sqllmsDonations = $dblms->querylms("SELECT d.id, d.status, d.amount, d.duration, s.std_id, s.std_name, s.std_fathername, s.std_gender, 
                                            s.std_regno, s.std_photo, c.class_name, cs.section_name
...
while($valueDetail = mysqli_fetch_array($sqllmsDonations)) {
    ...
    $sonDaughterOf = '';
    if($valueDetail['std_gender'] == 'Male'){
        $sonDaughterOf = '<span style="color:#333;font-size: 1.3rem;">S/O</span><br>';
    } elseif($valueDetail['std_gender'] == 'Female'){
        $sonDaughterOf = '<span style="color:#333;font-size: 1.3rem;">D/O</span>';
    }
        ...
        <a href="donations.php?std='.$valueDetail['std_id'].'&don='.$valDonor['donor_id'].'">
            <section class="panel panel-featured panel-featured-primary" >
                <header class="panel-heading">
                    <center>
                        <p>'.$photo.'</p>
                        <h4 class="text text-primary">'.$valueDetail['std_name'].'<br>'.$sonDaughterOf.$valueDetail['std_fathername'].'</h4>
                    </center>
                </header>
