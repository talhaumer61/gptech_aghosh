...
$sqllmspaid	= $dblms->querylms("SELECT f.status, SUM(f.total_amount) as paid
									FROM ".FEES." f				   
									WHERE f.status = '1' AND f.id_type = '3' AND f.is_deleted != '1' AND f.id_donor = '".$valDonor['donor_id']."'
...
$sqllmspending	= $dblms->querylms("SELECT f.status, SUM(f.total_amount) as pending
								   FROM ".FEES." f
									WHERE f.status = '2' AND f.id_type = '3' AND f.is_deleted != '1' AND f.id_donor = '".$valDonor['donor_id']."'
									AND f.id_campus = '".$_SESSION['userlogininfo']['LOGINCAMPUS']."' $sql2");
...
<div class="row mt-none mb-md">
	<div class="col-sm-12 col-md-12 col-lg-3 bg bg-info card mb-sm">
		<i class="fa fa-money" aria-hidden="true"></i> Total Payable
...
<section class="panel panel-featured panel-featured-primary">
	...
	<div class="panel-body">
		...
		$sqllms	= $dblms->querylms("SELECT f.id
										...
										AND f.id_donor = '".$valDonor['donor_id']."' $sql2");
		//--------------------------------------------------
		...
		$lpm1 		    = $lastpage - 1;
		//--------------------------------------------------  
		$sqllms	= $dblms->querylms("SELECT f.status, f.challan_no, f.id_month, f.to_month, f.due_date, f.paid_date, f.total_amount, f.pay_mode, f.remaining_amount
										FROM ".FEES." f			   
										WHERE f.id != '' AND f.id_type = '3' 
										AND f.is_deleted != '1'  $sql2
										AND f.id_donor = '".$valDonor['donor_id']."'
