...
$sqllms	= $dblms->querylms("
								...
								WHERE 
									...
									AND
										reg_id  	= '".cleanvars($_GET['edit_id'])."'
								LIMIT 1
							");
$valDemand = mysqli_fetch_array($sqllms);
//===========================================================================================
$sqllmsclasses	= $dblms->querylms("
										SELECT 
											c.class_name
										FROM ".CLASSES." c  
										WHERE 
											...
											AND 
												c.class_status 		= '1'
											AND
												c.class_id = '".cleanvars($valDemand['id_class'])."'
									");
$valClassName = mysqli_fetch_array($sqllmsclasses);
//===========================================================================================
$sqllmsExamTerms	= $dblms->querylms("
										SELECT 
											  et.type_id
											, et.type_name
										FROM ".EXAM_TYPES." et
...
<div class="row">
	<div class="col-md-12">
		<section class="panel panel-featured panel-featured-primary">
			<form action="exam_registration_campus.php?edit_id='.cleanvars($_GET['edit_id']).'" class="mb-lg validate" enctype="multipart/form-data" method="post" accept-charset="utf-8" autocomplete="off">
...
echo '
											<option value="'.$valueExamTerms['type_id'].'" selected>'.$valueExamTerms['type_name'].'</option>
';
									}
									else
									{
echo '
											<option value="'.$valueExamTerms['type_id'].'">'.$valueExamTerms['type_name'].'</option>
';										
									}
								endwhile;
echo '
				...
				<footer class="panel-footer">
					<div class="row">
						<div class="col-md-12 text-right">
							...
							<a class="btn btn-default" href="exam_registration_campus.php">Cancel</a>
						</div>
					</div>
				</footer>
			</form>
...
<script>
	$(document).ready(function() {
		...
		{
			$.ajax({
				url: "include/campus/exam_registration_campus/ajax.php"
				, type: "POST"
				, data: {
							  idClass : idClass.val()
