<?php 
// If Coordinator Then Classes
$sqllmsCoord = $dblms->querylms("SELECT coordinator_classes, coordinator_for
								   FROM ".EMPLOYEES."
								   WHERE id_loginid_coord = '".cleanvars($_SESSION['userlogininfo']['LOGINIDA'])."'
								   LIMIT 1");
$valCoord = mysqli_fetch_array($sqllmsCoord);
if(!EMPTY($valCoord['coordinator_classes'])) {
	$data = 'there is data';
	$allottedClasses = $valCoord['coordinator_classes'];
	$coordinator_for = $valCoord['coordinator_for'];
	$query_gender = 'AND std_gender = "'.$coordinator_for.'" ';
	$query_gender_s = 'AND s.std_gender = "'.$coordinator_for.'" ';
} else {
...
<div class="dropdown-menu notification-menu" style="min-width: 290px;">
	...
	<div class="content">
		...
		$sqllms	= $dblms->querylms("SELECT not_id, not_title, dated, not_description
										FROM ".NOTIFICATIONS."
										WHERE not_status = '1' AND id_type = '2' AND to_coordinator = '1' AND is_deleted != '1' ORDER BY dated DESC LIMIT 6");
		//-----------------------------------------------------
		$count = 0;
