...
if(isset($_POST['submit_student'])){
	...
	if(mysqli_num_rows($sqllmscheck) > 0) {
		...
		// ================= PARENT LOGIN AUTO CREATE (ON STUDENT ADD) =================
		...
		if($sqllms) {
			// Make Parent Login
			$fatherCNIC = cleanvars($_POST['std_fathercnic']);
			$pass = "ags@786";
				...
				if(mysqli_num_rows($sqllmscheck)){
					...
					$dblms->querylms("
						INSERT INTO sms_admins(
							...
							'".(!empty($_POST['std_fathername']) ? $_POST['std_fathername'] : 'Parent Account')."', '".$_POST['std_whatsapp']."', '".$_SESSION['userlogininfo']['LOGINCAMPUS']."', NOW(), '".$_SESSION['userlogininfo']['LOGINIDA']."'
						)
					");
...
if(isset($_POST['changes_student'])){ 
	...
	$sqlOld = $dblms->querylms("
		SELECT std_fathercnic 
		FROM sms_students 
		WHERE std_id = '".$_POST['std_id']."' 
		LIMIT 1
	");
	$old = mysqli_fetch_array($sqlOld);
	$oldCNIC = $old['std_fathercnic'];
	$newCNIC = cleanvars($_POST['std_fathercnic']);
	...
	if($sqllms) {
		// ================= PARENT LOGIN AUTO CREATE (ON STUDENT UPDATE) =================

		if($oldCNIC != $newCNIC && !empty($newCNIC)){
			$salt = dechex(mt_rand(0, 2147483647)) . dechex(mt_rand(0, 2147483647));
			$pass = "ags@786";
			...
			if(mysqli_num_rows($sqllmscheck)){
				...
				if($row['is_deleted'] == 1){
					$dblms->querylms("
						UPDATE sms_admins 
						SET is_deleted = 0, adm_logintype = 4, adm_status = 1, adm_userpass = '".$password."', adm_salt = '".$salt."', date_modify = NOW(), id_modify = '".$_SESSION['userlogininfo']['LOGINIDA']."' 
						WHERE adm_id = '".$parentLoginID."'
					");
				}
				else{
					$_SESSION['msg']['title'] 	= 'Error';
					$_SESSION['msg']['text'] 	= 'Parent login already exists for CNIC: '.$newCNIC;
					$_SESSION['msg']['type'] 	= 'error';
				}
			...
			// 🔥 Update ALL students of new CNIC
			$dblms->querylms("
				UPDATE sms_students 
				SET id_loginid = '".$parentLoginID."'
				WHERE std_fathercnic = '".$newCNIC."'
			");
		}
