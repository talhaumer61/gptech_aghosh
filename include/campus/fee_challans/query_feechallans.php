...
//	CHALLAN PAYMENT
if(isset($_POST['challan_payment'])){ 
	...
	$sqllmsupdate  = $dblms->querylms("UPDATE ".FEES." SET 
														  status    	    = '1'
														, paid_date		    = '".cleanvars($paidDate)."'
														, paid_amount	    = '".cleanvars($_POST['paid_amount'])."'
														, pay_mode			= '".cleanvars($_POST['pay_mode'])."' 
														, note				= '".cleanvars($_POST['note'])."'
														, id_modify			= '".cleanvars($_SESSION['userlogininfo']['LOGINIDA'])."' 
														, date_modify	    = NOW()
	...
	if($sqllmsupdate) {
		// Make Log
		$remarks = 'Challan Payment for Challan #: '.cleanvars($_POST['challan_no']).'';
		$sqllmslog  = $dblms->querylms("INSERT INTO ".ACCOUNTS_LOGS." (
															id_user 				, 
															filename				, 
															action					,
