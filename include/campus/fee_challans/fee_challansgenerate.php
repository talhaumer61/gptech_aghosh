					...
					<div class="form-group">
						...
						<select data-plugin-selectTwo data-width="100%" name="id_class" id="id_class" required title="Must Be Required" class="form-control">
							<option value="">Select</option>';
								$sqllms	= $dblms->querylms("SELECT class_id, class_name, id_classgroup
																FROM ".CLASSES." 
																WHERE class_status = '1' AND is_deleted != '1' 
																ORDER BY class_id ASC");
								while($rowsvalues = mysqli_fetch_array($sqllms)){
									if($rowsvalues['class_id'] == $class){
										echo'<option value="'.$rowsvalues['class_id'].', '.$rowsvalues['id_classgroup'].'" selected>'.$rowsvalues['class_name'].'</option>';
									}else{
										echo'<option value="'.$rowsvalues['class_id'].','.$rowsvalues['id_classgroup'].'">'.$rowsvalues['class_name'].'</option>';
									}
								}
							echo'
						</select>
