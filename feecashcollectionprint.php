...
$queryFeeCollection  = $dblms->querylms("SELECT fcc.*, fee.id, std.*, (SELECT name FROM ".ADMISSIONS_INQUIRY." WHERE form_no = fee.inquiry_formno GROUP BY form_no) as name 
    ...
    <body>
        ...
        <table width="960" align="center" style="text-align: left; margin-top: 18px;">
            <thead>
                <tr>
                    <th width="40">NAME:</th>
                    <th class="b-b">'.(!empty($valueFeeCollection['std_name']) ? $valueFeeCollection['std_name'] : $valueFeeCollection['name']).'</th>
                </tr>
            </thead>
        </table> 
        <table width="960" align="center" style="text-align: left; margin-top: 18px;">
